import { TextSummarizerHero } from '@/components/tools/text-summarizer';
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'TextSummarizerPage.metadata' });
...
interface TextSummarizerPageProps {
  params: Promise<{ locale: Locale }>;
}

export default async function TextSummarizerPage({ params }: TextSummarizerPageProps) {
  const { locale } = await params;
  const tRelated = await getTranslations({ locale, namespace: 'TextSummarizerPage.blocks.related' });
  ...
  return (
    <>
      ...
      <div className="relative z-10 flex flex-col">
        <TextSummarizerHero />
        <AiDetectorFeaturesSection i18nNamespace="TextSummarizerPage.blocks.howItWorks" />
        <Features3Section i18nNamespace="TextSummarizerPage.blocks.useCases" />
        <DetectionProcessSection
          i18nNamespace="TextSummarizerPage.blocks.related"
          items={[
            {
              href: '/',
        ...
        />
        <FaqSection i18nNamespace="TextSummarizerPage.blocks.faq" />
      </div>
    </>
  );
}
